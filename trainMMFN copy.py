...
from gossipcop_dataset import gossipcop_dataset, collate_fn as gossip_collate_fn, clipmodel
...
def train():
    ...
    # train_set = merged_twitter_dataset(is_train=True)
    # validate_set = merged_twitter_dataset(is_train=False)
    ...
    train_loader = DataLoader(
        ...
        num_workers=8,
        collate_fn=gossip_collate_fn,  # Use gossip-specific batching
        shuffle=True
    )
    ...
    # loss_f_rumor = torch.nn.CrossEntropyLoss(weight=torch.tensor([1.0, 1.5]).float().to(device))
